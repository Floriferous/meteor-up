...
const log = (0, _debug.default)('mup:module:mongo');

function logs(api) {
  log('exec => mup mongo logs');
  const args = api.getArgs();
  const sessions = api.getSessions(['mongo']); // remove mongo from args sent to docker
...
function setup(api) {
  ...
  const list = _nodemiral.default.taskList('Setup Mongo');
...
function start(api) {
  ...
  const list = _nodemiral.default.taskList('Start Mongo');
...
function stop(api) {
  ...
  const list = _nodemiral.default.taskList('Stop Mongo');
...
async function status(api) {
  ...
  let {
    output: dockerStatus
  } = await api.runSSHCommand(server, 'docker inspect mongodb --format "{{json .}}"');
  const mongoCommand = '"JSON.stringify(db.runCommand({serverStatus: 1, metrics: 0, wiredTiger: 1}))"';
  let {
    output: mongoStatus
  } = await api.runSSHCommand(server, `docker exec mongodb mongo --eval ${mongoCommand} --quiet`);
  ...
  const upTime = new Date(dockerStatus.State.FinishedAt).getTime() - new Date(dockerStatus.Created).getTime();
