...
var _build = _interopRequireWildcard(require("./build.js"));
...
async function logs(api) {
  ...
  const sessions = await (0, _utils.getSessions)(api);
...
function setup(api) {
  ...
  const list = _nodemiral.default.taskList('Setup Meteor');
...
async function build(api) {
  ...
  const buildOptions = (0, _utils.getBuildOptions)(api);
  const rebuild = (0, _utils.shouldRebuild)(api);
  ...
  if (rebuild) {
    ...
    await (0, _build.default)(appPath, buildOptions, api.getVerbose(), api);
  }
}
...
async function push(api) {
  ...
  if ((0, _utils.shouldRebuild)(api)) {
    await (0, _bluebird.promisify)(_build.archiveApp)(buildOptions.buildLocation, api);
  }

  if (appConfig.docker.prepareBundleLocally) {
    return (0, _prepareBundle.prepareBundleLocally)(buildOptions.buildLocation, api);
  }

  const list = _nodemiral.default.taskList('Pushing Meteor App');
  ...
  if ((0, _prepareBundle.prepareBundleSupported)(appConfig.docker)) {
    let tag = 'latest';

    if (api.swarmEnabled()) {
      const data = await api.getServerInfo();
      tag = (0, _utils.currentImageTag)(data, appConfig.name) + 1;
    }
    ...
    list.executeScript('Prepare Bundle', {
      ...
      vars: {
        ...
        env: (0, _utils.escapeEnvQuotes)(appConfig.env),
        ...
        imagePrefix: (0, _utils.getImagePrefix)(privateDockerRegistry)
      }
  ...
  let sessions = api.getSessions(['app']); // If we are using a private registry,
...
function envconfig(api) {
  ...
  const list = _nodemiral.default.taskList('Configuring App');
  ...
  const env = (0, _utils.createEnv)(app, api.getSettings());
  const hostVars = {};
  Object.keys(app.servers).forEach(key => {
    ...
    if (app.servers[key].env) {
      hostVars[host] = {
        env: _objectSpread(_objectSpread({}, app.servers[key].env), {}, {
    ...
    if (app.servers[key].settings) {
      const settings = JSON.stringify(api.getSettingsFromPath(app.servers[key].settings));
...
async function start(api) {
  ...
  const list = _nodemiral.default.taskList('Start Meteor');

  if (swarmEnabled) {
    const currentService = await api.dockerServiceInfo(config.name);
    const serverInfo = await api.getServerInfo();
    const imageTag = (0, _utils.currentImageTag)(serverInfo, config.name); // TODO: make it work when the reverse proxy isn't enabled

    api.tasks.addCreateOrUpdateService(list, (0, _utils.createServiceConfig)(api, imageTag), currentService);
  } else {
...
function deploy(api) {
  log('exec => mup meteor deploy'); // validate settings and config before starting
  ...
  return api.runCommand('meteor.push').then(() => api.runCommand('default.reconfig'));
}
...
async function stop(api) {
  ...
  const list = _nodemiral.default.taskList('Stop Meteor');
...
async function restart(api) {
  const list = _nodemiral.default.taskList('Restart Meteor');
  ...
  if (api.swarmEnabled()) {
    api.tasks.addRestartService(list, {
      name: appConfig.name
    });
  } else {
    list.executeScript('Stop Meteor', {
...
async function debugApp(api) {
  const {
    servers,
    app
  } = api.getConfig();
  let serverOption = api.getArgs()[2]; // Check how many sessions are enabled. Usually is all servers,
  // but can be reduced by the `--servers` option

  const enabledSessions = api.getSessions(['app']).filter(session => session);

  if (!(serverOption in app.servers)) {
    if (enabledSessions.length === 1) {
      const selectedHost = enabledSessions[0]._host;
      serverOption = Object.keys(app.servers).find(name => servers[name].host === selectedHost);
    } else {
  ...
  const {
    output
  } = await api.runSSHCommand(server, `docker exec -t ${app.name} sh -c 'kill -s USR1 $(pidof -s node)'`); // normally is blank, but if something went wrong
...
async function destroy(api) {
  ...
  const list = _nodemiral.default.taskList('Destroy App');
...
async function status(api) {
  ...
  const servers = Object.keys(config.app.servers).map(key => _objectSpread(_objectSpread({}, config.servers[key]), {}, {
    name: key
  }));
  const results = await (0, _bluebird.map)(servers, server => (0, _status.getInformation)(server, config.app.name, api), {
    concurrency: 2
  });
  const urlResults = await (0, _bluebird.map)(servers, server => (0, _status.checkUrls)(server, config.app, api), {
    concurrency: 2
  });
  const display = new StatusDisplay(`Meteor Status - ${config.app.name}`);
  results.forEach((result, index) => {
    const urlResult = urlResults[index] || {};
    const section = display.addLine(`- ${result.host}: ${(0, _status.withColor)(result.statusColor, result.status)}`, result.statusColor);
    ...
    (0, _status.createPortInfoLines)(result.exposedPorts, result.publishedPorts, section);
    (0, _status.displayAvailability)(result, urlResult, section);
  });
