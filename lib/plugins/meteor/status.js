...
exports.createPortInfoLines = createPortInfoLines;
exports.withColor = withColor;
exports.displayAvailability = displayAvailability;
...
function getInformation(server, appName, api) {
  return api.runSSHCommand(server, `docker inspect ${appName} --format "{{json .}}"`) // eslint-disable-next-line complexity
  .then(({
    host,
    output
  }) => {
...
async function checkUrlLocally(server, appConfig, port) {
  ...
  if (appConfig.env.VIRTUAL_HOST) {
    domain = appConfig.env.VIRTUAL_HOST.split(',')[0]; // TODO: this should use the proxy's port if the user changed it
  ...
  try {
    result = await _axios.default.head(`${protocol}${domain}${portString}`, {
      timeout: 5000
    });
  } catch (e) {
    result = false;
...
function getCheckAddress(server, appConfig) {
  if (appConfig.servers && appConfig.servers[server.name] && appConfig.servers[server.name].bind) {
...
async function checkUrls(server, appConfig, api) {
  const port = appConfig.servers[server.name].env ? appConfig.servers[server.name].env.PORT : appConfig.env.PORT;
  const [remote, inDocker, local] = await Promise.all([api.runSSHCommand(server, `curl ${getCheckAddress(server, appConfig)}:${port}`), api.runSSHCommand(server, `docker exec ${appConfig.name} curl http://localhost:${appConfig.docker.imagePort}`), checkUrlLocally(server, appConfig, port)]);
...
function createPortInfoLines(exposedPorts = [], publishedPorts = [], statusDisplay) {
...
function displayAvailability(result, urlResult, statusDisplay) {
